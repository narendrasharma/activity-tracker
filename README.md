...
```sql
CREATE DATABASE activity_tracker CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
export DB_NAME=activity_tracker
export DB_USER=root
...
**Apache** — the included `.htaccess` handles everything. Ensure `AllowOverride All` is set.
...
server {
    listen 80;
    server_name activity-tracker.local;
...
Navigate to `http://localhost/activity-tracker/public/index.html` (or your configured domain).
...
- `.htaccess` blocks direct access to `.json`, `.sql`, and `.log` files

---

## 📊 Performance

- Indexes on `user_id`, `action`, `created_at`, `ip_address`, and composite columns
- Pagination prevents full-table scans on large datasets
- `MAX_LIMIT` cap (500) prevents runaway queries
- Top-users response cached for 2 minutes to avoid repeated aggregation queries
- `ROW_FORMAT=COMPRESSED` on the main table for storage efficiency
...
curl -X POST http://localhost/activity-tracker/api/log.php \
  -H "Content-Type: application/json" \
  -d '{"user_id":1,"action":"test","metadata":{"key":"value"}}'

# Fetch logs
curl "http://localhost/activity-tracker/api/logs.php?limit=10"

# Top users
curl "http://localhost/activity-tracker/api/top-users.php"

# Anomalies
curl "http://localhost/activity-tracker/api/anomalies.php"

# Export CSV
curl "http://localhost/activity-tracker/api/logs.php?export=csv" -o logs.csv
```
...
- Ensure `cache/data/` and `cache/rate_limits/` are writable on the server
- Run `sql/schema.sql` before first use
- Seed test data with `php cli/seed.php --count=10000 --anomalies`

---

## 👤 Author

Built as a technical interview task demonstrating Core PHP OOP, MySQL query design, security practices, and system architecture without any frameworks.
